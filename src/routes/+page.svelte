		...
		<div class="w-full max-w-4xl mx-auto px-4 pt-8 pb-16 flex flex-col gap-4">
			...
			<header class="rounded-sm border border-white/8 bg-white/3 p-4">
				<div class="flex items-stretch gap-3 flex-wrap">
					<div class="shrink-0 min-h-16 flex items-center justify-center rounded-sm border border-white/8 bg-white/5 px-2">
						<img src={user.avatar_url} alt={user.login} class="w-12 h-12 rounded-sm" />
					</div>
					<div class="flex-1 min-h-16 flex flex-col justify-center rounded-sm border border-white/8 bg-white/5 px-3">
						<div class="font-semibold text-white text-lg leading-tight whitespace-nowrap">{user.name ?? user.login}</div>
						<a href={user.html_url} target="_blank" rel="noopener"
							class="text-sm text-white/50 hover:text-white whitespace-nowrap">@{user.login}</a>
					...
					{#if siteRepo}
						<div class="shrink-0 min-h-16 flex flex-col justify-center rounded-sm border border-white/8 bg-white/5 px-3">
					...
					<!-- Compact stat strip -->
					<div class="flex gap-4 shrink-0 min-h-16 items-center rounded-sm border border-white/8 bg-white/5 px-4">
					...
					{#if WEBSITE}
						<a href={`https://${WEBSITE}`} target="_blank" rel="noopener"
							class="shrink-0 min-h-16 flex items-center gap-1.5 rounded-sm border border-white/8 bg-white/5 px-3 text-xs text-white/70 hover:border-white/20 hover:text-white transition-all">
