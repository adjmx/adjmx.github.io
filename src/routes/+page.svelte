<script lang="ts">
	...
	async function loadRepoDetail(full: string, ownLogin: string): Promise<RepoDetail | null> {
		...
		if (own) {
			...
			html_url = own.html_url;
			stars = own.stargazers_count;
			forks = own.forks_count;
			updated = own.updated_at;
		} else {
			try {
	...
	function describe(e: GHEvent): Activity | null {
		...
		switch (e.type) {
			...
			case 'PullRequestEvent': {
				...
				const verb =
					a === 'closed' ? (merged ? 'Merged' : 'Closed') : a.charAt(0).toUpperCase() + a.slice(1);
				return {
					icon: 'pr',
					action: `${verb} PR #${num} in`,
					repo,
			...
			case 'CreateEvent': {
				...
				if (t === 'repository')
					return { icon: 'branch', action: 'Created', repo, detail: null, created_at: at };
				return {
					icon: 'branch',
					action: `Created ${t} ${e.payload?.ref ?? ''} in`,
					repo,
					detail: null,
					created_at: at
				};
			...
			case 'ReleaseEvent':
				return {
					icon: 'tag',
					action: `Released ${e.payload?.release?.tag_name ?? ''} in`,
					repo,
					detail: null,
					created_at: at
				};
	...
	const ICONS: Record<string, string> = {
		commit:
			'<circle cx="12" cy="12" r="3.5"/><path d="M12 3v5.5M12 15.5V21" stroke-width="2" stroke="currentColor" fill="none"/>',
		...
		issue:
			'<circle cx="12" cy="12" r="8.5" stroke-width="2" stroke="currentColor" fill="none"/><circle cx="12" cy="12" r="2.5"/>',
		branch:
			'<circle cx="6" cy="6" r="2.5"/><circle cx="6" cy="18" r="2.5"/><circle cx="18" cy="7" r="2.5"/><path d="M6 8.5v7M18 9.5c0 4-5 2.5-5 6.5" stroke-width="2" stroke="currentColor" fill="none"/>',
...
<main class="flex min-h-screen flex-col">
	<!-- Nav -->
	<nav class="flex items-center justify-between border-b border-white/10 px-6 py-3">
	...
	{#if loading}
		<div class="flex flex-col items-center justify-center gap-4 py-20">
			<div
				class="h-10 w-10 animate-spin rounded-full border-2 border-white/20"
				style="border-top-color: var(--accent)"
			></div>
		</div>
	{:else if user}
		<div class="mx-auto flex w-full max-w-4xl flex-col gap-4 px-4 pt-8 pb-16">
			<!-- Profile header + compact stat strip -->
			<header class="rounded-sm border border-white/8 bg-white/3 p-4">
				<div class="flex flex-wrap items-stretch gap-3">
					<div
						class="flex min-h-16 shrink-0 items-center justify-center rounded-sm border border-white/8 bg-white/5 px-2"
					>
						<img src={user.avatar_url} alt={user.login} class="h-12 w-12 rounded-sm" />
					</div>
					<div
						class="flex min-h-16 flex-1 flex-col justify-center rounded-sm border border-white/8 bg-white/5 px-3"
					>
						<div class="text-lg leading-tight font-semibold whitespace-nowrap text-white">
						...
						<a
							...
							target="_blank"
							rel="noopener"
							class="text-sm whitespace-nowrap text-white/50 hover:text-white">@{user.login}</a
						>
						{#if user.bio}<p class="mt-1 text-sm text-white/60">{user.bio}</p>{/if}
					...
					{#if siteRepo}
						<div
							class="flex min-h-16 shrink-0 flex-col justify-center rounded-sm border border-white/8 bg-white/5 px-3"
						>
							<div class="mb-1 text-[9px] tracking-wider text-white/35 uppercase">This site</div>
							<a
								href={siteRepo.html_url}
								target="_blank"
								rel="noopener"
							...
							{#if siteRepo.language}
								<div class="mt-1 flex items-center gap-1 text-[10px] text-white/40">
									<span
										...
										style="background:{langColor(siteRepo.language)}"
									></span>
									{siteRepo.language}
								</div>
							{/if}
					...
					<!-- Compact stat strip -->
					<div
						class="flex min-h-16 shrink-0 items-center gap-4 rounded-sm border border-white/8 bg-white/5 px-4"
					>
						<div class="text-center">
							<div class="text-lg leading-none font-bold text-white">{user.public_repos}</div>
							<div class="mt-1 text-[10px] tracking-wide text-white/40 uppercase">Repos</div>
						</div>
						<div class="text-center">
							<div class="text-lg leading-none font-bold text-white">{user.followers}</div>
							<div class="mt-1 text-[10px] tracking-wide text-white/40 uppercase">Followers</div>
						</div>
						<div class="text-center">
							<div class="text-lg leading-none font-bold text-white">{user.following}</div>
							<div class="mt-1 text-[10px] tracking-wide text-white/40 uppercase">Following</div>
						</div>
						<div class="text-center">
							<div class="text-lg leading-none font-bold text-white">{totalStars}</div>
							<div class="mt-1 text-[10px] tracking-wide text-white/40 uppercase">Stars</div>
						</div>
					</div>
					...
					{#if WEBSITE}
						<a
							...
							target="_blank"
							rel="noopener"
							class="flex min-h-16 shrink-0 items-center gap-1.5 rounded-sm border border-white/8 bg-white/5 px-3 text-xs text-white/70 transition-all hover:border-white/20 hover:text-white"
						>
							<svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
								><path
									stroke-linecap="round"
					...
					>
						<svg class="h-3.5 w-3.5" fill="currentColor" viewBox="0 0 24 24"
							><path
								d="M12 0C5.37 0 0 5.37 0 12c0 6.63 5.37 12 12 12s12-5.37 12-12S18.63 0 12 0zm0 22c-5.52 0-10-4.48-10-10S6.48 2 12 2s10 4.48 10 10-4.48 10-10 10z"
							/></svg
						>
						Nostr
					</a>
				...
				{#if languages.length > 0}
					<div class="mt-3 flex items-center gap-2 border-t border-white/8 pt-3 text-xs">
						<span class="shrink-0 text-white/30">Languages</span>
						<span class="flex h-2 flex-1 overflow-hidden rounded-full bg-white/5">
							{#each languages as l}
								<span style="width:{l.pct}%; background:{langColor(l.name)}"></span>
							{/each}
						</span>
						<span class="max-w-[45%] shrink-0 truncate text-white/50"
							>{languages.map((l) => l.name).join(' · ')}</span
						>
					</div>
				{/if}
			</header>
			...
			<div class="grid grid-cols-1 items-start gap-4 md:grid-cols-[1fr_1.4fr]">
				<!-- Projects -->
				<section class="rounded-sm border border-white/8 bg-white/3 p-4">
					<h2 class="mb-3 text-sm font-semibold tracking-wider text-white/50 uppercase">
						Projects
					</h2>
					<div class="grid grid-cols-1 gap-2.5">
						{#each visibleRepos as repo}
							<a
								...
								target="_blank"
								rel="noopener"
								class="group block rounded-sm border border-white/8 bg-white/3 p-3 transition-all hover:border-white/20"
							>
								<div class="flex items-start justify-between gap-2">
									<span
										class="truncate text-sm font-medium transition-colors group-hover:[color:var(--accent)]"
										style="color:#e6edf3">{repo.name}</span
									>
									<span class="shrink-0 text-[10px] text-white/30">{rel(repo.updated_at)}</span>
								</div>
								{#if repo.description}
									<p class="mt-1 line-clamp-2 text-xs leading-snug text-white/50">
										{repo.description}
									</p>
								{/if}
								<div class="mt-2 flex items-center gap-3 text-[11px] text-white/40">
									{#if repo.language}
										<span class="inline-flex items-center gap-1">
											<span
									...
									{#if repo.stargazers_count > 0}
										<span class="inline-flex items-center gap-0.5">
											<svg class="h-3 w-3" fill="currentColor" viewBox="0 0 24 24"
												><path
													d="M12 3l2.6 5.6 6 .8-4.4 4.2 1.1 6-5.3-3-5.3 3 1.1-6L3.4 9.4l6-.8z"
												/></svg
											>
										...
										<span class="inline-flex items-center gap-0.5">
											<svg class="h-3 w-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"
												><circle cx="6" cy="6" r="2.5" /><circle cx="18" cy="6" r="2.5" /><circle
													...
													cy="18"
													r="2.5"
												/><path stroke-width="2" d="M6 8.5c0 4 6 3 6 7M18 8.5c0 4-6 3-6 7" /></svg
											>
											{repo.forks_count}
										</span>
									{/if}
					...
					{#if otherRepos.length > 6}
						<div class="mt-3 text-center">
							<button
								...
								class="rounded-sm border border-white/10 px-3.5 py-1.5 text-xs text-white/60 transition-all hover:border-white/20 hover:text-white"
							>
				...
				<section class="rounded-sm border border-white/8 bg-white/3 p-4">
							...
							{#each activity as a}
								<div class="flex gap-2.5">
									<svg
										...
										fill="currentColor"
										viewBox="0 0 24 24">{@html ICONS[a.icon] ?? ICONS.commit}</svg
									>
									<div class="min-w-0 text-xs leading-snug">
										...
										{#if a.detail}<div class="mt-0.5 truncate text-white/40">{a.detail}</div>{/if}
					...
					{#if activeDetails.length > 0}
						<div class="mt-4 border-t border-white/8 pt-4">
							<h3 class="mb-2.5 text-[11px] font-semibold tracking-wider text-white/40 uppercase">
								In these repos
							</h3>
									...
									<div class="rounded-sm border border-white/8 bg-white/3 p-3">
										<div class="flex items-start justify-between gap-2">
											<a
												...
												target="_blank"
												rel="noopener"
												class="truncate text-sm font-medium [color:var(--accent)] hover:underline"
												>{d.external ? d.full : d.name}</a
											>
											<div class="flex shrink-0 items-center gap-2 text-[10px] text-white/30">
												{#if d.external}<span class="rounded bg-white/5 px-1.5 py-0.5 text-white/40"
														>starred</span
													>{/if}
												{#if d.updated_at}<span>{rel(d.updated_at)}</span>{/if}
											</div>
										</div>
										{#if d.description}<p class="mt-1 text-xs leading-snug text-white/50">
												{d.description}
											</p>{/if}
										{#if d.langs.length > 0}
											<span class="mt-2.5 flex h-1.5 overflow-hidden rounded-full bg-white/5">
												{#each d.langs.slice(0, 6) as l}<span
														style="width:{l.pct}%; background:{langColor(l.name)}"
													></span>{/each}
											</span>
											<div
												class="mt-1.5 flex flex-wrap gap-x-2.5 gap-y-1 text-[10px] text-white/45"
											>
												{#each d.langs.slice(0, 4) as l}
													<span class="inline-flex items-center gap-1"
														...
														></span>{l.name}
														{Math.round(l.pct) || '<1'}%</span
													>
												{/each}
											</div>
										{/if}
										{#if d.stargazers_count > 0 || d.forks_count > 0}
											<div class="mt-2 flex items-center gap-3 text-[10px] text-white/35">
												{#if d.stargazers_count > 0}<span class="inline-flex items-center gap-0.5"
														><svg class="h-3 w-3" fill="currentColor" viewBox="0 0 24 24"
															><path
																d="M12 3l2.6 5.6 6 .8-4.4 4.2 1.1 6-5.3-3-5.3 3 1.1-6L3.4 9.4l6-.8z"
															/></svg
														>{d.stargazers_count}</span
													>{/if}
												{#if d.forks_count > 0}<span class="inline-flex items-center gap-0.5"
														><svg
															class="h-3 w-3"
															fill="none"
															stroke="currentColor"
															...
															><circle cx="6" cy="6" r="2.5" /><circle
																...
																cy="6"
																r="2.5"
															/><circle cx="12" cy="18" r="2.5" /><path
																stroke-width="2"
																d="M6 8.5c0 4 6 3 6 7M18 8.5c0 4-6 3-6 7"
															/></svg
														>{d.forks_count}</span
													>{/if}
											</div>
	...
	{:else if rateLimited}
		<section class="flex flex-col items-center justify-center gap-2 px-4 pt-20 pb-8 text-center">
			<p class="text-white/60">GitHub's API rate limit was reached.</p>
			<p class="text-sm text-white/35">
				This page reads live data from the public GitHub API (60 requests/hour per IP,
				unauthenticated). Please try again in a few minutes.
			</p>
		</section>
	{:else}
	...
	<footer class="mt-auto pb-6 text-center text-sm text-white/25">
		<a
			href="https://svelte.dev"
			class="inline-flex items-center gap-2 transition-colors hover:text-white/50"
		>
			<img
				src="{base}/svelte1.png"
				alt="Svelte"
				class="h-4 w-4 opacity-40 transition-opacity hover:opacity-70"
			/>
			Built with Svelte
		</a>
	</footer>
